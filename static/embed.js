...
(function () {
  ...
  style.textContent = [
    ...
    "width:min(420px,calc(100vw - 48px))!important;height:min(720px,calc(100dvh - 48px))!important;",
  ...
  function detectPageLang() {
    try {
      ...
      if (/^\/en(-[a-z]{2})?(\/|$)/.test(path) || /\/en(\/|$)/.test(path)) return "en";
      var body = (document.body && document.body.className) || "";
      if (/\b(lang-en|pll-en|wpml-en)\b/i.test(body)) return "en";
      if (/wp-wpml_current_language=en/.test(document.cookie || "")) return "en";
      var host = (location.hostname || "").toLowerCase();
      if (host.indexOf("en.") === 0 || host.indexOf(".en.") !== -1) return "en";
    } catch (e) {}
    return "pl";
  }
  ...
  tip.id = "bdj-ai-tip";
  var tipText = PAGE_LANG === "en"
    ? "Hi! I can help you pick spare parts 😊"
    : "Cześć! Chętnie pomogę Ci w doborze części 😊";
  var tipClose = PAGE_LANG === "en" ? "Close" : "Zamknij";
  tip.innerHTML = '<div style="display:flex;gap:10px;align-items:flex-start;"><div style="flex:1;min-width:0;"><strong>DRAGON AI</strong><span>' + tipText + '</span></div><button id="bdj-ai-tip-close" type="button" aria-label="' + tipClose + '">&times;</button></div>';
